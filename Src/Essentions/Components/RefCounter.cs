...
namespace Essentions.Components
{
    ...
    ///     it is time for an item to be released. Thread-safe. Contains maximum of <see cref="int.MaxValue" /> items.
    /// </summary>
    ...
    public class RefCounter<TItem>
    {
            ...
            if (_refs.TryGetValue(item, out itemRef))
                lock (itemRef) {
                    if (itemRef.Count > 0) {
                        ...
                        if (itemRef.IsZero() && _refs.TryRemove(item, out removedRef)) Released.Call(item);
                    }
                }
        }
        ...
        public bool Tracked([CanBeNull] TItem item)
        {

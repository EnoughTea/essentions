...
namespace Essentions.Tests.IO
{
    public sealed class GlobberTests
    {
        public sealed class TheConstructor
        {
            ...
            {
                // Given, When
                var result = Assert.Catch(() => new Globber(null));

                // Then
                AssertEx.IsArgumentNullException(result, "env");
            }
        }
        ...
        public sealed class TheMatchMethod
        {
            public sealed class WindowsSpecific
            {
                ...
                public void Should_Throw_If_Unc_Root_Was_Encountered()
                {
                    ...
                    // When
                    var result = Assert.Catch(() => fixture.Match("//Foo/Bar/Qux.c"));

                    // Then
                    Assert.That(result, Is.TypeOf<NotSupportedException>());
                    Assert.AreEqual("UNC paths are not supported.", result.Message);
                }
                ...
                public void Should_Ignore_Case_Sensitivity_On_Case_Insensitive_Operative_System()
                {
                    ...
                    // Then
                    Assert.AreEqual(1, result.Length);
                    Assert.That(result[0], Is.TypeOf<FilePath>());
                    AssertEx.ContainsFilePath(result, "C:/Working/Foo/Bar/Qux.c");
                }
            ...
            public void Should_Throw_If_Pattern_Is_Null()
            {
                ...
                // When
                var result = Assert.Catch(() => fixture.Match(null));

                // Then
                Assert.That(result, Is.TypeOf<ArgumentNullException>());
                Assert.That(((ArgumentNullException)result).ParamName, Is.EqualTo("pattern"));
            }
            ...
            public void Should_Throw_If_Visiting_Parent_That_Is_Recursive_Wildcard()
            {
                ...
                // When
                var result = Assert.Catch(() => fixture.Match("/Working/Foo/**/../Foo/Bar/Qux.c"));

                // Then
                Assert.NotNull(result);
                Assert.That(result, Is.TypeOf<NotSupportedException>());
                Assert.AreEqual("Visiting a parent that is a recursive wildcard is not supported.", result.Message);
            }
